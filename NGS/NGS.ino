/**************************************************************************
    This pinball code for Nitro Ground Shakers is distributed in the hope that it
...
const unsigned long SuperBonusCountDelayMs = 1000;
...
void setup()
{
    ...
    DropTargets.DefineSwitch(0, SW_DROP_TARGET1);
    DropTargets.DefineSwitch(1, SW_DROP_TARGET2);
    DropTargets.DefineSwitch(2, SW_DROP_TARGET3);
    ...
    for (byte scoreCount = 0; scoreCount < 4; scoreCount++)
    {
        ...
        else
        {
            ...
            if (allScoresShowValue == 0)
            {
                ...
                if (CurrentAchievements[scoreCount])
...
void IncreasePlayfieldMultiplier(unsigned long duration)
{
    if (PlayfieldMultiplierExpiration)
        PlayfieldMultiplierExpiration += duration;
    else
        PlayfieldMultiplierExpiration = CurrentTime + duration;
                ...
                *CurrentAdjustmentByte = curVal;
                ...
                else if (curState == MACHINE_STATE_ADJUST_MUSIC_VOLUME)
                {
                    if (SoundSettingTimeout)
                        Audio.StopAllAudio();
                    Audio.PlaySound(SOUND_EFFECT_BACKGROUND3, AUDIO_PLAY_TYPE_WAV_TRIGGER, curVal);
                ...
                else if (curState == MACHINE_STATE_ADJUST_SFX_VOLUME)
                {
                    if (SoundSettingTimeout)
                        Audio.StopAllAudio();
                    Audio.PlaySound(SOUND_EFFECT_SPINNER, AUDIO_PLAY_TYPE_WAV_TRIGGER, curVal);
                ...
                else if (curState == MACHINE_STATE_ADJUST_CALLOUTS_VOLUME)
                {
                    if (SoundSettingTimeout)
                        Audio.StopAllAudio();
                    Audio.PlaySound(SOUND_EFFECT_EXTRA_BALL, AUDIO_PLAY_TYPE_WAV_TRIGGER, curVal);
...
void QueueNotification(unsigned int soundEffectNum, byte priority)
{
    ...
    if (SoundSelector < 3 || SoundSelector == 4 || SoundSelector == 7 || SoundSelector == 9)
        return;
    if (soundEffectNum < SOUND_EFFECT_VP_VOICE_NOTIFICATIONS_START || soundEffectNum >= (SOUND_EFFECT_VP_VOICE_NOTIFICATIONS_START + NUM_VOICE_NOTIFICATIONS))
        return;
...
int RunAttractMode(int curState, boolean curStateChanged)
{
    ...
    if (curStateChanged)
    {
        ...
        if (RPU_ReadSingleSwitchState(SW_CENTER_SAUCER))
        {
            RPU_PushToSolenoidStack(SOL_CENTER_SAUCER, 16, true);
        }
...
int InitGamePlay(boolean curStateChanged)
{
    ...
    */
    if (RPU_ReadSingleSwitchState(SW_CENTER_SAUCER))
    {
        ...
        {
            RPU_PushToSolenoidStack(SOL_CENTER_SAUCER, 12, true);
    ...
    if (RPU_ReadSingleSwitchState(SW_LEFT_SAUCER))
    {
        if (CurrentTime > (SaucerEjectTime + 2500))
        {
            RPU_PushToSolenoidStack(SOL_LEFT_SAUCER, 12, true);
            SaucerEjectTime = CurrentTime;
        }
    }
...
void StartBonusCountdown(boolean isEndOfBall = false)
{
    ...
    if (BallState.collectLit[BONUS_LANE_RIGHT] || isEndOfBall)
    {
        ...
        if (CurrentTime >= BonusCountdownTickTime)
        {
            ...
            if (BonusCountPhase == BONUS_LANE_RIGHT)
            {
                ...
                if (BallState.laneBonus[BONUS_LANE_RIGHT] == 0)
                {
                    ...
                    {
                        // Right bonus is done counting, switch to left if it's lit or this is end of ball
                        if ((BallState.collectLit[BONUS_LANE_LEFT] || isEndOfBall) &&
                            BallState.laneBonus[BONUS_LANE_LEFT] > 0)
                        {
                            BonusCountPhase = BONUS_LANE_LEFT;
                            BonusCountdownTickTime = CurrentTime + BonusCountdownDelayMs;
                            CountBonusAgain = BallState.doubleBonus;
            ...
            else
            {
                for (byte count = 0; count < 4; count++)
                {
                    if (count != CurrentPlayer)
                        OverrideScoreDisplay(count, PlayfieldMultiplier, DISPLAY_OVERRIDE_ANIMATION_BOUNCE);
                }
                DisplaysNeedRefreshing = true;
            }
        }
...
int BallEndCountdownBonusState(boolean curStateChanged)
{
    ...
    if (curStateChanged)
    {
        StartBonusCountdown(true);
    }

    boolean finished = CountdownBonus(true);
...
int ShowMatchSequence(boolean curStateChanged)
{
    ...
    if (curStateChanged)
    {
        MatchSequenceStartTime = CurrentTime + 4500;
        MatchDelay = 1500;
